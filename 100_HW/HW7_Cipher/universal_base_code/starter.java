...
class starter {
	public static void main(String args[]) {
       ...
       System.out.println(Cipher.encode("6v lvp u5vuy5 y1z5 qpsqy5r"));
        ...
        String message1 = new String("u4y i1e i5 i6ita5c 314c1i66e1 i5f by6i5 hea5c zbi58 u4y");
        String message2 = new String("amlb mfhp7fcj jlgq fo tap s9a bjlslql aflipa lb hlg9b 210z t9ct");

        int key1 = 0;
        while (key1 <= 36) {
            System.out.println("Key " + key1 + ": " + Cipher.keyedEncode(message1, key1));
            key1++;
        }

        int key2 = 0;
        while (key2 <= 36) {
            System.out.println("Key " + key2 + ": " + Cipher.keyedEncode(message2, key2));
            key2++;
        }
         System.out.println("make a code and ill encode it");
        String myMessage = sc.nextLine();
        String encoded = Cipher.encode(myMessage);
