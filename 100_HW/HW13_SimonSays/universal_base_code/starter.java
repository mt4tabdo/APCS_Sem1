...
class starter {
    ...
    while(true){
        ...
        String[] seq = new String[level];
        for(int i = 0; i < level; i++){
            seq[i] = getColorWord();
            Colors.println(seq[i]); // print each color once
        }
        ...
        String[] ans = new String[level];
        for(int i = 0; i < level; i++){
            ans[i] = sc.nextLine();
        }

        boolean correct = true; // only declare once
        for(int i = 0; i < level; i++){
            if(!ans[i].equals(seq[i])){
                correct = false;
            }
        }
        ...
        level++;
            ...
            */
            ...
            while(true){
                System.out.println("--------------------------------------");
                String[] seqWords = new String[level];
                String[] seqTextColors = new String[level];
                for(int i = 0; i < level; i++){
                    seqWords[i] = colors[rand.nextInt(colors.length)];
                    seqTextColors[i] = colors[rand.nextInt(colors.length)];
                    Colors.println(seqWords[i], "BRIGHT_" + seqTextColors[i]);
                }
                System.out.println("\n--------------------------------------");
                System.out.println("Memorize the phrase above!");
                System.out.print("You have : ");
                ...
                System.out.println("Please enter the correct word or text color for each line:");
                String[] ans = new String[level];
                for(int i = 0; i < level; i++){
                    ans[i] = sc.nextLine();
                }
                ...
                for(int i = 0; i < level; i++){
                    if(rand.nextBoolean()){
                        if(!ans[i].equals(seqWords[i])) correct = false;
                    } else {
                        if(!ans[i].equals(seqTextColors[i])) correct = false;
                    }
                }
                if(!correct){
                    System.out.println("You got it wrong! Try again!");
    ...
    while(true){
        ...
        String[] seqWords = new String[level];
        String[] seqTextColors = new String[level];
        String[] seqBgColors = new String[level];

        for(int i = 0; i < level; i++){
            seqWords[i] = colors[rand.nextInt(colors.length)];
            seqTextColors[i] = colors[rand.nextInt(colors.length)];
            seqBgColors[i] = colors[rand.nextInt(colors.length)];

            // Display word with both text and background color
            Colors.println(seqWords[i], "BRIGHT_" + seqTextColors[i], "BG_" + seqBgColors[i]);
        }
        ...
        boolean correct = true;
        for(int i = 0; i < level; i++){
            if(!ans[i].equals(seqTextColors[i])){
                correct = false;
            }
        }
